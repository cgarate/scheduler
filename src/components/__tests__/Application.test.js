...
describe("Application", () => {
  ...
  it("loads data, deletes an interview and increases the spots remaining for Monday by 1", async () => {
    ...
    const appointmentToCancel = getAllByTestId(
      container,
      "appointment",
    ...
    expect(
      getByTestId(appointmentToCancel, "icon-add-appointment"),
    ).toBeInTheDocument();
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // Get only the second article with appointment data
    const appointmentToEdit = getAllByTestId(
      container,
      "appointment",
    ).find(appointment => queryByText(appointment, "Archie Cohen"));

    // Select the icon we need to click on to edit an appointment and click on it
    const icon = getByAltText(appointmentToEdit, "Edit");
    ...
    // Get the Save button
    const saveButton = getByText(appointmentToEdit, "Save");
    ...
    await waitForElementToBeRemoved(() =>
      getByText(appointmentToEdit, "Saving..."),
    );

    expect(getByText(container, "Guy Incognito")).toBeInTheDocument();

    // Check the counter is the same
    expect(
      getByText(mondayDayListItems, "1 spot remaining"),
    ).toBeInTheDocument();
  });
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    expect(
      getByText(
        rootAppointmentArticle,
        "Oopsie Daysies! I could not save that appointment!",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    expect(
      getByText(
        appointmentToCancel,
        "Oopsie Daysies! I could not delete the appointment!",
      ),
    ).toBeInTheDocument();
  });
});
